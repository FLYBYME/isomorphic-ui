...
export class RouterModule implements IMeshModule {
    ...
    async onReady(): Promise<void> {
        ...
        const state = this.app?.getProvider<ReactiveState<any>>('state');
        if (state && !state.data.$router) {
            state.data.$router = {
    ...
    private async handleNavigation(): Promise<void> {
            ...
            if (params) {
                ...
                if (route.guards) {
                    for (const guard of route.guards) {
                        const passed = await guard({} as any); // Simplified context
                        if (!passed) return;
                    }
                }

                // Update ReactiveState
                state.data.$router.current = {
                    path,
        ...
        console.warn(`[RouterModule] No route matched for path: ${path}`);
    }
